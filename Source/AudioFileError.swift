...
/**
 ...
 An enum with cases that represent the result codes defined by Audio File Services.
...
public enum AudioFileError: CodedErrorType {
    ...
    public init(status: OSStatus, message: String) {
        ...
        case kAudioFileUnsupportedFileTypeError:
            self = .UnsupportedFileType(message: message)
        case kAudioFileUnsupportedDataFormatError:
            self = .UnsupportedDataFormat(message: message)
        case kAudioFileUnsupportedPropertyError:
            self = .UnsupportedProperty(message: message)
        case kAudioFileBadPropertySizeError:
            self = .BadPropertySize(message: message)
        case kAudioFilePermissionsError:
            self = .Permissions(message: message)
        case kAudioFileNotOptimizedError:
            self = .NotOptimized(message: message)
        case kAudioFileInvalidChunkError:
            self = .InvalidChunk(message: message)
        case kAudioFileDoesNotAllow64BitDataSizeError:
            self = .DoesNotAllow64BitDataSize(message: message)
        case kAudioFileInvalidPacketOffsetError:
            self = .InvalidPacketOffset(message: message)
        case kAudioFileInvalidFileError:
            self = .InvalidFile(message: message)
        case kAudioFileOperationNotSupportedError:
            self = .OperationNotSupported(message: message)
        case kAudioFileNotOpenError:
            self = .NotOpen(message: message)
        case kAudioFileEndOfFileError:
            self = .EndOfFile(message: message)
        case kAudioFilePositionError:
            self = .Position(message: message)
        case kAudioFileFileNotFoundError:
            self = .FileNotFound(message: message)
        default:
            self = .Undefined(status: status, message: message)
        }
    }
    ...
    public var message: String {
        ...
        case .Unspecified(let m):
            return m
        case .UnsupportedFileType(let m):
            return m
        case .UnsupportedDataFormat(let m):
            return m
        case .UnsupportedProperty(let m):
            return m
        case .BadPropertySize(let m):
            return m
        case .Permissions(let m):
            return m
        case .NotOptimized(let m):
            return m
        case .InvalidChunk(let m):
            return m
        case .DoesNotAllow64BitDataSize(let m):
            return m
        case .InvalidPacketOffset(let m):
            return m
        case .InvalidFile(let m):
            return m
        case .OperationNotSupported(let m):
            return m
        case .NotOpen(let m):
            return m
        case .EndOfFile(let m):
            return m
        case .Position(let m):
            return m
        case .FileNotFound(let m):
            return m
    ...
    public var shortDescription: String {
        ...
        case .DoesNotAllow64BitDataSize:
            return "Does not allow 64-bit data size."
        case .InvalidPacketOffset:
            return "Invalid packet offset."
        case .InvalidFile:
            return "Invalid file."
        case .OperationNotSupported:
        ...
        case .Undefined:
            return "This error is not defined by Audio File Services."
        }
    }
}